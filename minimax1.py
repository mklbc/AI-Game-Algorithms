...
def minimax(position, depth, is_maximizing, node_count=0):
    ...
    if is_maximizing:
        ...
        for child in get_children(position):
            eval, node_count = minimax(child, depth - 1, False, node_count)
            max_eval = max(max_eval, eval)
        return max_eval, node_count
    else:
        min_eval = np.inf
        for child in get_children(position):
            eval, node_count = minimax(child, depth - 1, True, node_count)
